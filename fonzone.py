...
# ===== НАСТРОЙКИ =====
TOKEN = "8397567369:AAFki44pWtxP5M9iPGEn26yvUsu1Fv-9g3o"  # Замените на токен от @BotFather
CRYPTO_BOT_API_KEY = "498509:AABNPgPwTiCU9DdByIgswTvIuSz5VO9neRy"  # Получите в @CryptoBot через /pay
ADMIN_IDS = [7577716374]  # Ваш Telegram ID
...
invoices = {}
premium_users = set()
support_messages = {}  # Для переписки с поддержкой
user_contacts = {}  # Контакты пользователей
broadcast_data = {}  # Для хранения данных рассылки
...
"""
        ...
        if photos:
            if len(photos) == 1:
                ...
                msg = bot.send_media_group(CHANNEL_ID, media)
...
def broadcast_to_list(user_ids, message_text, message_type='text', photo=None, admin_id=None):
    ...
    for i, user_id in enumerate(user_ids, 1):
        try:
            ...
            if message_type == 'photo' and photo:
                bot.send_photo(user_id_int, photo, caption=message_text)
            else:
                bot.send_message(user_id_int, message_text)
            
            successful += 1
            
            # Отправляем прогресс каждые 5 пользователей
...
"""
    ...
    msg3 = bot.send_message(user_id, "Выберите действие:", reply_markup=keyboard)
    save_message_id(user_id, msg3.message_id)
        ...
        if ad.get('photos') and len(ad['photos']) > 0:
            try:
                if len(ad['photos']) == 1:
                    ...
                    msg = bot.send_media_group(user_id, media)
                    msg2 = bot.send_message(user_id, "Действия с объявлением:", reply_markup=keyboard)
...
def select_condition(call):
    ...
    ad_drafts[user_id]['condition'] = conditions.get(condition, 'Не указано')
    user_states[user_id]["previous"].append(user_states[user_id]["current"])
...
@bot.message_handler(content_types=['text', 'photo', 'video', 'document'], func=lambda m: m.from_user.id in ADMIN_IDS and broadcast_data.get(m.from_user.id))
...
• /admin_invoices - список платежей
...
def cmd_admin_users(message):
    ...
    users_list = sorted(users_data.items(), 
                       ...
                       reverse=True)[:20]  # Показываем последние 20
...
if __name__ == '__main__':
    ...
    print("С ИНТЕГРАЦИЕЙ CRYPTOBOT API И РАССЫЛКОЙ")
    print("=" * 50)
